...
window._a = function moduleApp() {
    ...
    map = L.map('map-id').setView([12.3603119, -1.5163034], 7);
    //}
    ...
    var createCustomIcon = function createCustomIcon(feature, latlng) {
        var myIcon = L.icon({
            ...
            iconSize: [38, 40], // width and height of the image in pixels
            shadowSize: [65, 30], // width, height of optional shadow image
            iconAnchor: [12, 12], // point of the icon which will correspond to marker's location
            shadowAnchor: [12, 6], // anchor point of the shadow. should be offset
            popupAnchor: [10, 10] // point from which the popup should open relative to the iconAnchor
        });
        return L.marker(latlng, { icon: myIcon }).bindPopup('Ouagadougou').openPopup();
    };
    ...
    var oneMapMouseOver = function oneMapMouseOver(e) {
        ...
        if (parseInt(nouveau) > 0) popupContent += "<div><span class='' style='color:#ffc107; font-size:1.5rem;'>" + nouveau + "</span> cas confirmé(s)</div>";

        if (parseInt(guerison) > 0) popupContent += "<div><span class='' style='color:#2db756; font-size:1.5rem;'>" + guerison + "</span> guérison(s) </div>";

        if (parseInt(deces) > 0) popupContent += "<div><span class='' style='color:#a63f3d; font-size:1.5rem;'>" + deces + "</span> décès </div>";

        popupContent += "<p class='text-muted' style='text-align:center;font-style:italic'>A la date du <" + "em class='text-muted'>- <u>" + dated + "</u> -</p>";
    ...
    var stat2Chart = Highcharts.chart('stat2', {
        ...
        series: [{
            ...
            data: [{
                ...
                color: colorN /*Highcharts
                              .getOptions()
                              .colors[0] // Jane's color*/
            }, {
                name: 'Guérisons',
                y: 0,
                color: colorG /*Highcharts
                              .getOptions()
                              .colors[2] // Joe's color*/
            }, {
                name: 'Décès',
                y: 0,
                color: colorD /*Highcharts
                              .getOptions()
                              .colors[1] // John's color*/
            }],
            center: [100, 80],
            size: 100,
            showInLegend: false,
    ...
    var confirmeChart = Highcharts.chart('confirmer', {
        ...
        tooltip: {
            pointFormat: '{series.name}: {point.y:.f} cas'
        },
        plotOptions: {
            pie: {
    ...
    var initMap = function initMap(data) {
        var regions = JSON.parse(data.region);
        var villes = data.ville;

        regionsG = L.geoJson(regions, { style: style }).addTo(map);

        villesG = L.geoJson(villes, {
            pointToLayer: createCustomIcon,
            onEachFeature: onMapEachFeature
        }).addTo(map);
    ...
    var processData = function processData(donnee) {
        ...
        stat2Chart.xAxis[0].update({ categories: datas.jourdates });
        stat2Chart.series[0].update({ data: datas.jourconfrimes });
        stat2Chart.series[1].update({ data: datas.jourgueris });
        stat2Chart.series[2].update({ data: datas.jourdeces });
        stat2Chart.series[3].data[0].update({
            y: parseInt(datas.sommeconfrimes)
        });
        stat2Chart.series[3].data[1].update({
            y: parseInt(datas.sommegueris)
        });
        stat2Chart.series[3].data[2].update({
        ...
        confirmeChart.series[0].update({ data: casconfirmesparville });
        ...
        guerirChart.series[0].update({ data: casguerisparville });
        ...
        decederChart.series[0].update({ data: decesparville });
    };
                ...
                if (localStorage['mdata'] && !bl) {
                    ...
                    $.ajax({
                        ...
                        success: function success(data) {
                            ...
                            localStorage.setItem('lst', Math.round(new Date().getTime() / 1000));
                            initMap(data);
                        }
                    });
                    ...
                    $.ajax({
                        ...
                        success: function success(donnee) {
 ...
 */
e.channel('covidstatChannel').listen('.covidstat.newDeclaration', function (e) {
